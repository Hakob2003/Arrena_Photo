    ...
    try {
      ...
      const response = await axios.post(url, requestBody, {
        headers: {
          ...
          ...(isOpenRouter
            ? {
                "HTTP-Referer": "https://arrenaphoto.com",
                "X-Title": "ArrenaPhoto",
              }
            : {}),
        },
      ...
      if (isOpenRouter) {
        if (
          ...
          if (
            ...
            (!imageUrl.startsWith("/") || imageUrl.length > 500)
          ) {
      ...
      if (error.response) {
        throw Object.assign(
          new Error(
            `${isOpenRouter ? "OpenRouter" : "OpenAI"} API error: ${error.response.data?.error?.message || error.response.statusText}`,
          ),
          { cause: error },
        );
      }
