...
export class EncryptionUtil {
  ...
  private static getKey(): Buffer {
    ...
    if (!keyString || keyString.length !== 32) {
      throw new Error('ENCRYPTION_KEY must be exactly 32 characters long in environment variables.');
    }
  ...
  public static decrypt(encryptedText: string): string {
    ...
    // Graceful degradation for legacy base64 keys
    if (!encryptedText.startsWith(this.PREFIX)) {
      try {
        return Buffer.from(encryptedText, 'base64').toString('utf8');
      } catch (e) {
        return encryptedText; // if it was plain text or something else
      }
    }
