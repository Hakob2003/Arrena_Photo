...
async function bootstrap() {
  ...
  if (!jwtSecret || jwtSecret.length < 32) {
    logger.fatal(
      "JWT_SECRET is missing or too short (must be at least 32 characters). Shutting down for security reasons.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    // Enable trust proxy so that rate limiting works correctly behind Render's load balancer
    ...
    app.enableCors({
      origin: [
        ...
        process.env.FRONTEND_URL ||
          "https://arrena-photo-frontend-o4xg.onrender.com",
      ],
