  ...
  async create(userId: string, dto: CreateGenerationDto) {
    ...
    const [planConfig, user] = await Promise.all([
      ...
      this.prisma.user.findUnique({
        where: { id: userId },
        select: {
          maxConcurrentOverride: true,
          queueDelayOverride: true,
    ...
    const maxTasks =
      user?.maxConcurrentOverride ??
      planConfig?.maxConcurrent ??
      (plan === "FREE" ? 1 : 2);
    ...
    if (activeTasks >= maxTasks) {
      throw new BadRequestException(
        `Concurrent generation limit reached for your plan (${plan}). Max allowed: ${maxTasks}`,
      );
    }
    ...
    let queueDelay =
      user?.queueDelayOverride ?? planConfig?.queueDelay ?? 30000;
    ...
    try {
      await this.generationsQueue.add(
        "generate-image",
        {
