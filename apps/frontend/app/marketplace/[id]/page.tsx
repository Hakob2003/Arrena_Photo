        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4">
            ...
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`relative w-24 h-24 flex-shrink-0 bg-gray-200 dark:bg-gray-800 rounded-xl overflow-hidden cursor-pointer hover:ring-2 transition-all ${isLuxury ? "ring-[#D4AF37]" : "ring-blue-500"}`}
              >
                <Image
                  src={template.coverUrl}
                  alt="Gallery item"
        ...
        <div className="flex flex-col justify-center space-y-6">
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-slate-400 dark:text-gray-500">
              ...
              <span>•</span>
              <span>
                {template.downloads.toLocaleString("en-US")} downloads
              </span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm flex justify-between items-center">
            <div>
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full ${isLuxury ? "bg-gradient-to-tr from-[#C5A028] to-[#D4AF37]" : "bg-gradient-to-tr from-blue-500 to-purple-500"}`}
                ></div>
                <div>
                  <p className="font-bold">{template.author.name}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-400">
                    {template.author.followers} followers
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="pt-6 border-t border-gray-200 dark:border-gray-800 flex gap-2 sm:gap-4 sticky bottom-4 z-10 bg-transparent/80 sm:bg-transparent backdrop-blur-none sm:backdrop-blur-none p-4 sm:p-0 rounded-2xl sm:rounded-none">
            <button
              className={`flex-1 flex justify-center items-center py-3 sm:py-4 rounded-xl font-bold text-base sm:text-lg shadow-lg transition-all transform hover:-translate-y-1 ${
                isLuxury
                  ? "bg-[#D4AF37] hover:bg-[#C5A028] text-black shadow-[#D4AF37]/30"
            ...
            >
              ...
              {template.oldPrice !== null &&
                ...
                template.price > template.oldPrice && (
                  <span title={`Previously $${template.oldPrice}`}>
                    <ArrowUp
                      className={`w-5 h-5 ml-2 ${isLuxury ? "text-[#C5A028]" : "text-red-500"}`}
                    />
                  </span>
                )}
              {template.oldPrice !== null &&
                ...
                template.price < template.oldPrice && (
                  <span title={`Previously $${template.oldPrice}`}>
                    <ArrowDown
                      className={`w-5 h-5 ml-2 ${isLuxury ? "text-[#D4AF37]" : "text-green-400"}`}
                    />
                  </span>
                )}
            </button>
