      ...
      <div className="p-8 w-full animate-fade-in">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 mb-8">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="h-10 w-24 bg-black/[0.03] dark:bg-white/5 animate-pulse rounded-full flex-shrink-0"
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="aspect-[4/5] rounded-2xl bg-black/[0.03] dark:bg-white/5 animate-pulse border border-black/10 dark:border-white/10"
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex gap-4 overflow-x-auto pb-2 sm:pb-0 custom-scrollbar flex-1">
          <button
            ...
            className={`px-5 py-2 rounded-full whitespace-nowrap transition-colors ${
              selectedCategory === "__all__"
                ? isLuxury
                  ? "bg-[#D4AF37] shadow-none text-black"
          ...
          {categories
            .filter((c) => templates.some((t) => t.categoryId === c.id))
            .map((c) => (
              <button
                key={c.id}
                onClick={() => {
        ...
        {mounted && (
          <div className="flex-shrink-0 ml-auto sm:ml-0 flex items-center gap-4">
            ...
            >
              <option value={5} className="dark:bg-[#0A0A0A]">
                5 {t("templates.perPage")}
              </option>
              <option value={10} className="dark:bg-[#0A0A0A]">
                10 {t("templates.perPage")}
              </option>
              <option value={20} className="dark:bg-[#0A0A0A]">
                20 {t("templates.perPage")}
              </option>
            </select>
          ...
          {currentTemplates.map((tpl, i) => {
            ...
            return (
              <motion.div
                ...
                onClick={() =>
                  (window.location.href = `/generate?templateId=${tpl.id}`)
                }
                className="group glass-card rounded-2xl overflow-hidden cursor-pointer relative"
              >
                {tpl.price !== undefined && tpl.price !== null && (
                  <div
                    className={`absolute top-4 right-4 z-20 backdrop-blur-none px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-lg ${
                      isLuxury
                        ? "bg-[#D4AF37]/90 text-black shadow-none"
                        : "bg-indigo-600 shadow-[0_8px_24px_rgba(99,102,241,0.25)] dark:shadow-none/90 text-white"
                    }`}
                  >
                    <span>⚡ {tpl.price}</span>
                ...
                <div className="aspect-[4/5] relative overflow-hidden bg-black/[0.03] dark:bg-white/5">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 z-20">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white mt-1">
                      {tpl.name}
                    </h3>
                  </div>
                </div>
