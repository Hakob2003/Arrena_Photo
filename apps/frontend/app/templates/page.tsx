...
export default function TemplatesPage() {
      ...
      <div className="p-8 w-full animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
            <div key={i} className="aspect-[4/5] rounded-2xl bg-muted animate-pulse border border-border" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="p-8 w-full">
      <div className="mb-10">
        <h1 className="text-4xl font-bold mb-4">Шаблоны промптов</h1>
        <p className="text-muted-foreground text-lg">Начните создание с идеально составленных промптов.</p>
      </div>
      ...
      {filteredTemplates.length === 0 ? (
        <div className="text-center py-20 text-muted-foreground glass-card rounded-2xl p-8">
            ...
            return (
              ...
              >
                {t.price !== undefined && t.price !== null && (
                  <div className="absolute top-4 right-4 z-20 bg-indigo-600/90 backdrop-blur-md px-2.5 py-1 rounded-full text-xs font-bold text-foreground flex items-center gap-1 shadow-lg">
                    <span>⚡ {t.price}</span>
                  </div>
                )}
