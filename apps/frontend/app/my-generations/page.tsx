...
export default function MyGenerationsPage() {
  ...
  const saveToDrive = async (
    ...
    try {
      setSavingId(id);
      await api.post("/integrations/google-drive/save", {
        imageUrl,
        generationId: id,
      });
      ...
      setGenerations((prev) =>
        prev.map((g) => (g.id === id ? { ...g, driveFileId: "saved" } : g)),
      );
    } catch (error: any) {
      if (
        error.response?.status === 400 &&
        error.response?.data?.message?.includes("not connected")
      ) {
  ...
  const togglePublish = async (
    ...
    try {
      ...
      toast.success(
        currentStatus ? t("myGen.unpublished") : t("myGen.published"),
      );
      setGenerations((prev) =>
        prev.map((g) => (g.id === id ? { ...g, isPublic: !currentStatus } : g)),
      );
    } catch (error) {
      ...
      <div className="p-8 w-full h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-2">
            {t("myGen.loginRequired")}
          </h2>
          <p className="text-slate-500 dark:text-gray-400 mb-6">
            {t("myGen.loginDescription")}
          </p>
          <a
            href="/login"
    ...
    <div className="p-4 sm:p-8 w-full">
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          <div
            className={`w-12 h-12 border-4 border-black/10 dark:border-white/10 rounded-full animate-spin ${isLuxury ? "border-t-[#D4AF37]" : "border-t-indigo-500"}`}
          />
        </div>
        ...
        <div className="flex-1 border border-black/10 dark:border-white/10 rounded-2xl bg-white/5 backdrop-blur-none p-10 flex flex-col items-center justify-center text-center">
          <div className="w-20 h-20 bg-black/[0.03] dark:bg-white/5 rounded-full flex items-center justify-center text-4xl mb-6">
            🎨
          </div>
          ...
          <a
            href="/generate"
            className={`px-6 py-3 glass rounded-xl font-bold hover:bg-black/[0.05] dark:bg-white/10 transition-colors ${
              isLuxury
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {generations.map((gen) => (
            ...
            >
              <AuthImage
                driveFileId={
                  gen.driveFileId !== "saved" ? gen.driveFileId : undefined
                }
                fallbackUrl={gen.imageUrl}
                alt="Generated"
                className="w-full h-full object-cover group-hover:scale-105 transition-transform"
              />
              <div className="absolute inset-0 bg-transparent/60 opacity-0 group-hover:opacity-100 transition-opacity p-3 flex flex-col justify-between">
                <div className="flex justify-end">
                  ...
                  <button
                    ...
                    title={
                      gen.isPublic ? "Unpublish from Feed" : "Publish to Feed"
                    }
                  >
                    {publishingId === gen.id ? (
                      <div className="w-4 h-4 border-2 border-black/20 dark:border-white/20 border-t-white rounded-full animate-spin" />
                ...
                <div>
                  <span
                    className={`text-xs font-bold mb-1 block ${isLuxury ? "text-[#D4AF37]" : "text-indigo-400"}`}
                  >
                    {gen.model}
                  </span>
                  {gen.template && (
                    <span className="text-xs text-gray-300 block">
                      {t("myGen.template")}: {gen.template}
                    </span>
                  )}
                  <span className="text-xs text-slate-400 dark:text-gray-500 block mt-1">
