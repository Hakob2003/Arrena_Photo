...
export default function AiConnectionsPage() {
  ...
  return (
    <div className="p-4 sm:p-8 w-full">
      <h1 className="text-3xl sm:text-4xl font-bold mb-2">{t('ai.title')}</h1>
      <p className="text-slate-500 dark:text-gray-400 mb-10">{t('ai.description')}</p>
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <span className={`text-xs px-2 py-1 rounded-md font-bold uppercase ${
                ...
                {p.status === 'connected' ? t('ai.connected') : t('ai.disconnected')}
              </span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 flex justify-end">
                <button className={`w-full sm:w-auto px-4 py-3 sm:py-2 rounded-lg text-sm font-bold transition-colors min-h-[44px] ${
                  ...
                  {p.status === 'connected' ? t('ai.disconnect') : t('ai.connect')}
                </button>
              </div>
            </div>
          </motion.div>
