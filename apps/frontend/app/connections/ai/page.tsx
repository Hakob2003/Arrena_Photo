...
export default function AiConnectionsPage() {
  ...
  return (
    <div className="p-4 sm:p-8 w-full">
      ...
      <p className="text-muted-foreground mb-10">Подключите свои API-ключи или локальные инстансы для генерации без траты кредитов платформы.</p>
        ...
        {providers.map((p, i) => (
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-muted-foreground uppercase font-semibold mb-1 block">API Key</label>
                <input 
                  type="password" 
              ...
              <div className="pt-2 flex justify-end">
                <button className={`w-full sm:w-auto px-4 py-3 sm:py-2 rounded-lg text-sm font-bold transition-colors min-h-[44px] ${
                  p.status === 'connected' 
                  ? 'bg-muted/50 hover:bg-red-500/20 hover:text-red-400 text-muted-foreground' 
