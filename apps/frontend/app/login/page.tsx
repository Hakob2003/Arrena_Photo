...
function LoginContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!err.response) {
        errorMessage =
          "Network error. The server might be down or waking up. Please wait a minute and try again.";
      } else if (
        ...
        errorMessage =
          "Server is waking up (Render free tier). Please wait ~60 seconds and try again.";
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center p-4">
      <div className="w-full max-w-md p-6 sm:p-8 bg-white dark:bg-[#0a0a0a] glass-card rounded-2xl shadow-xl border border-black/10 dark:border-white/10">
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-100 text-red-600 border border-red-200 rounded-lg text-sm">
            {error}
          </div>
        )}
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2 border rounded-lg focus:ring-2 outline-none bg-slate-50 dark:bg-[#111] text-slate-900 dark:text-white transition-colors ${
                isLuxury
                  ? "border-black/10 dark:border-white/10 focus:border-[#D4AF37] focus:ring-[#D4AF37]/20"
                  : "border-black/10 dark:border-white/10 focus:border-indigo-500 focus:ring-indigo-500/20"
              }`}
              value={email}
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
          <button
            ...
            className="py-2.5 bg-white dark:bg-[#111] border border-black/10 dark:border-white/10 text-slate-700 dark:text-gray-300 rounded-lg flex justify-center items-center hover:bg-slate-50 dark:hover:bg-white/5 transition text-sm font-medium"
          >
            Google
          </button>
          <button
        ...
        <p className="mt-6 text-center text-sm text-slate-600 dark:text-gray-400">
          ...
          <Link
            href="/register"
            className={`font-medium hover:underline transition-colors ${isLuxury ? "text-[#D4AF37]" : "text-indigo-600 dark:text-indigo-400"}`}
          >
            {t("auth.registerLink")}
          </Link>
        </p>
