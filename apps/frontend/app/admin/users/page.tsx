...
export default function AdminUsersPage() {
  ...
  const columns = [
    columnHelper.accessor('id', {
      header: 'ID',
      cell: info => <span className="text-slate-400 dark:text-gray-500 font-mono text-xs">{info.getValue().slice(0,8)}...</span>,
    }),
      ...
      cell: info => {
        ...
        return (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-slate-400 dark:text-gray-500">{row.lastLogin ? new Date(row.lastLogin).toLocaleDateString() : 'Never'}</span>
          </div>
        );
      }
    }),
    columnHelper.accessor('plan', {
      header: 'Plan',
      cell: info => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${info.getValue() === 'FREE' ? 'bg-white dark:bg-gray-800 text-slate-500 dark:text-gray-400' : 'bg-blue-500/20 text-blue-400'}`}>
          {info.getValue()}
        </span>
      ),
    }),
      ...
      cell: info => {
        ...
        return (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-slate-400 dark:text-gray-500 font-mono">{row.generationCount || 0} gens</span>
          </div>
        );
      }
    }),
        ...
        return (
          <div className="flex flex-wrap gap-2">
            ...
            <button 
              ...
              className="text-xs bg-red-900/50 hover:bg-red-800 text-slate-900 dark:text-white px-2 py-1 rounded transition-colors"
            >
              Delete
            </button>
          </div>
      ...
      <div className="mt-8 relative overflow-hidden bg-[#0a0a0a] border border-black/10 dark:border-white/10 rounded-2xl">
        <div className="overflow-x-auto w-full">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={columns.length} className="px-6 py-8 text-center text-slate-400 dark:text-gray-500">Loading...</td>
              </tr>
            ) : data.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="px-6 py-8 text-center text-slate-400 dark:text-gray-500">No users found</td>
              </tr>
            ) : (
              table.getRowModel().rows.map(row => (
                <tr key={row.id} className="border-b border-black/5 dark:border-white/5 hover:bg-white/[0.02] transition-colors">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="px-3 py-1 bg-[#0a0a0a] border border-black/10 dark:border-white/10 rounded text-sm disabled:opacity-50 hover:bg-black/[0.03] dark:bg-white/5"
          >
            Previous
          </button>
          <button 
