    ...
    <div className="mb-6 bg-gradient-to-br from-amber-500/5 to-orange-500/5 border border-amber-500/20 rounded-2xl p-5">
      ...
      <div className="space-y-2">
        {PROVIDER_GUIDES.map(provider => {
          ...
          return (
            <div key={provider.name} className={`border rounded-xl overflow-hidden transition-colors ${isOpen ? provider.color : 'border-black/10 dark:border-white/10 bg-white/[0.02]'}`}>
              {/* Header */}
              <button
              ...
              >
                ...
                <svg className={`w-4 h-4 text-slate-500 dark:text-gray-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-4">
                  {/* Steps */}
                  <div>
                    <h4 className="text-xs text-slate-500 dark:text-gray-400 uppercase tracking-wider mb-2 font-medium">Пошаговая инструкция</h4>
                      ...
                      {provider.steps.map((step, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm">
                          {step.startsWith('💡') || step.startsWith('⚠️') ? (
                            ...
                            <>
                              <span className="w-5 h-5 shrink-0 bg-black/[0.05] dark:bg-white/10 rounded-full flex items-center justify-center text-xs text-slate-500 dark:text-gray-400 font-mono mt-0.5">{i + 1}</span>
                  ...
                  {/* Models Table */}
                  <div>
                    <h4 className="text-xs text-slate-500 dark:text-gray-400 uppercase tracking-wider mb-2 font-medium">Рекомендуемые модели</h4>
                        ...
                        <tbody className="divide-y divide-white/5">
                          {provider.models.map(m => (
                            <tr key={m.slug} className="hover:bg-white/[0.03]">
                              ...
                              <td className="px-3 py-2 text-slate-500 dark:text-gray-400 max-w-[200px] truncate">{m.desc}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
  ...
  return (
    <>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors flex items-center gap-2 border ${
                showHelp ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' : 'bg-black/[0.03] dark:bg-white/5 border-black/10 dark:border-white/10 text-slate-500 dark:text-gray-400 hover:text-white hover:bg-black/[0.05] dark:bg-white/10'
              }`}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl p-4">
          <p className="text-xs text-slate-400 dark:text-gray-500 uppercase tracking-wider mb-1">Провайдеров</p>
          <p className="text-2xl font-bold text-indigo-400">{providers.length}</p>
        </div>
        <div className="bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl p-4">
          <p className="text-xs text-slate-400 dark:text-gray-500 uppercase tracking-wider mb-1">Бесплатных</p>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <span className="text-slate-500 dark:text-gray-400">Загрузка...</span>
                    </div>
                  </td>
                </tr>
              ) : models.length === 0 ? (
                  ...
                  <tr key={model.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <div className="text-slate-400 dark:text-gray-500 text-xs font-mono mt-0.5">{model.slug}</div>
                        {model.description && (
                          <div className="text-slate-400 dark:text-gray-500 text-xs mt-0.5 max-w-[250px] truncate">{model.description}</div>
                        )}
                      </div>
                    </td>
                    {/* Status */}
      ...
      {/* Modal */}
      {showModal && (
        <div className="fixed inset-0 bg-transparent/70 backdrop-blur-none z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div 
          ...
          >
            ...
            <div className="px-6 py-4 space-y-4">
              {/* Provider */}
              <div>
                <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Провайдер *</label>
                <select
              ...
              {/* Name */}
              <div>
                <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Название модели *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              {/* Slug */}
              <div>
                <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Slug (уникальный ID) *</label>
                <input
                  type="text"
                  value={form.slug}
              ...
              {/* Description */}
              <div>
                <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Описание</label>
                <textarea
              ...
              {/* Endpoint */}
              <div>
                <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Endpoint URL</label>
                <input
                  type="text"
                  value={form.endpoint}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Стоимость $/1k токенов</label>
                  <input
                    type="number"
                    step="0.0001"
                    min="0"
                ...
                <div>
                  <label className="block text-xs text-slate-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Скорость</label>
                  <select
                    value={form.speed}
