...
export default function ApiKeysAdminPage() {
  ...
  const [monitorConfigProvider, setMonitorConfigProvider] =
    useState<ProviderWithKey | null>(null);
  ...
  const handleSaveKey = async (providerId: string) => {
    ...
    try {
      await api.post(`/admin/api-providers/${providerId}/key`, {
        apiKey: keyValue,
      });
  ...
  const handleCheckConnection = async (providerId: string) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Ошибка проверки соединения",
      );
      fetchProviders();
    } finally {
      setCheckingId(null);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "CONNECTED":
        return <Badge variant="success">Подключен</Badge>;
      case "ERROR":
        return <Badge variant="error">Ошибка</Badge>;
      default:
        return <Badge variant="default">Не проверен</Badge>;
    }
  };

  const formatBalance = (balance: number | null) => {
    if (balance === null)
      return <span className="text-slate-400 dark:text-gray-500">Н/Д</span>;
    return (
      <span className="text-green-400 font-mono">${balance.toFixed(4)}</span>
    );
  };
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8 max-w-full overflow-hidden">
      ...
      >
        <div className="bg-transparent rounded-2xl overflow-hidden flex flex-col min-h-0 flex-1 w-full max-w-full">
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="font-medium text-slate-900 dark:text-slate-900 dark:text-white flex items-center gap-2">
                          {p.name}
                          {p.isGlobal && (
                            <span className="text-[10px] uppercase tracking-wider bg-black/[0.05] dark:bg-white/10 text-gray-300 px-1.5 py-0.5 rounded">
                              Global
                            </span>
                          )}
                        </div>
                        ...
                        {editingId === p.id ? (
                          <div className="flex items-center gap-2 max-w-[300px]">
                            <input
                              ...
                              autoFocus
                              onKeyDown={(e) =>
                                e.key === "Enter" && handleSaveKey(p.id)
                              }
                            />
                            <button
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-500 dark:text-gray-400 hover:bg-black/[0.05] dark:bg-white/10 rounded"
                            >
                              <svg
                                className="w-4 h-4"
                                fill="none"
                      ...
                      {/* Balance */}
                      <td className="px-6 py-4">{formatBalance(p.balance)}</td>
                          ...
                          {p.isAutoMonitorOn ? (
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-green-400 font-medium">
                                Вкл ({getIntervalLabel(p.monitorInterval)})
                              </span>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1.5">
