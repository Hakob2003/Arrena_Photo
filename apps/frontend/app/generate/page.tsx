...
function GeneratorContent() {
  ...
  const fetchHistory = React.useCallback(async () => {
    ...
    try {
      ...
      if (typeof meRes.data.credits === "number")
        setCredits(meRes.data.credits);
      if (
        meRes.data.planId &&
        meRes.data.planId !== useAuthStore.getState().planId
      ) {
  ...
  React.useEffect(() => {
    const loadTemplateData = async () => {
      if (templateId) {
        ...
        setPrompt(
          `Style of ${templateName}, detailed, masterpiece, 8k resolution, highly realistic...`,
        );
      } else if (
        useGenerationStore.getState().isPremiumTemplate &&
        useGenerationStore.getState().activeTemplateCost !== null
      ) {
      ...
      if (remixId) {
        try {
          ...
          if (gen) {
            ...
            if (gen.settings?.resolution)
              setResolution(gen.settings.resolution);
            if (gen.settings?.aspectRatio)
              setAspectRatio(gen.settings.aspectRatio);
    ...
    const fetchModels = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          ...
          const matchedModel = res.data.find(
            (m: any) =>
              m.id === currentModel ||
              m.slug === currentModel ||
              m.name === currentModel,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        elapsed += 1500;
        setProgress(
          Math.min(99, Math.floor((elapsed / estimatedTotalTime) * 100)),
        );
        if (elapsed > 60000) {
          if (pollRef.current) clearInterval(pollRef.current);
          setGenerating(false);
          ...
          if (status === "PROCESSING") {
            ...
            setTimeout(() => {
              if (statusRes.data.result) {
                setResult(
                  statusRes.data.result.imageUrl,
                  statusRes.data.result.driveFileId,
                );
              } else {
      ...
      if (
        ...
        err.response?.data?.message === "Insufficient credits"
      ) {
    ...
    <div className="h-full flex flex-col lg:flex-row gap-3 sm:gap-4 xl:gap-6 p-3 sm:p-4 xl:p-6 overflow-y-auto lg:overflow-hidden">
      ...
      <div className="w-full lg:w-[320px] xl:w-[380px] 2xl:w-[400px] lg:shrink-0 flex flex-col gap-4 sm:gap-6 overflow-y-auto pb-2 pr-1 custom-scrollbar">
          ...
          >
            ...
            {initImage ? (
              <div className="relative group">
                ...
                >
                  <span className="text-slate-900 dark:text-white font-bold text-sm bg-red-500/80 px-3 py-1 rounded-full cursor-pointer">
                    {t("gen.remove")}
                  </span>
                </div>
              </div>
              ...
              <div className="text-slate-500 dark:text-gray-400 py-6">
                <p>{t("gen.dropzone")}</p>
                <p className="text-xs mt-2 text-slate-400 dark:text-gray-500">
                  {t("gen.dropzoneFormats")}
                </p>
              </div>
            )}
          </div>
        ...
        >
          {isGenerating
            ? t("gen.generating")
            : `${t("gen.createButton")} ⚡ ${cost} ${t("gen.creditsUnit")}`}
        </button>
      </div>
      ...
      <div className="flex-1 min-h-[400px] lg:min-h-0 glass-card rounded-2xl flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 min-h-0 min-w-0 flex items-center justify-center p-3 sm:p-4 lg:p-6 preview-area-compact bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white/[0.02] to-transparent">
          {isGenerating ? (
            <div className="flex flex-col items-center gap-4 sm:gap-6 w-full max-w-sm">
              ...
              >
                <div
                  className={`absolute inset-0 border-4 border-black/10 dark:border-white/10 rounded-full animate-spin ${isLuxury ? "border-t-[#D4AF37]" : "border-t-indigo-500"}`}
                />
                <span
                  className={`text-lg font-bold ${isLuxury ? "text-[#D4AF37]" : "text-indigo-400"}`}
                >
                  {progress}%
                </span>
              </div>
        ...
        <div className="h-32 sm:h-36 lg:h-40 xl:h-44 shrink-0 gallery-container-compact border-t border-black/10 dark:border-white/10 bg-white/5 p-3 sm:p-4 overflow-x-auto overflow-y-hidden whitespace-nowrap">
          <h3 className="text-[10px] sm:text-xs font-bold gallery-title-compact text-slate-500 dark:text-gray-400 mb-2 uppercase tracking-wider">
            {t("gen.galleryTitle")}
          </h3>
          <div className="flex w-max gap-2 sm:gap-4 h-[60px] sm:h-[70px] lg:h-[80px] xl:h-[90px] gallery-items-compact">
            {history.length === 0 ? (
              <div className="text-slate-400 dark:text-gray-500 text-sm flex items-center h-full">
                {t("gen.galleryEmpty")}
              </div>
            ) : (
              history.map((item) => (
                <div
                  key={item.id}
                  className="relative aspect-square h-full shrink-0 rounded-lg overflow-hidden group cursor-pointer"
                  onClick={() => setResult(item.imageUrl, item.driveFileId)}
                >
                  <AuthImage
                  ...
                  <div className="absolute inset-0 bg-transparent/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2">
                    <span className="text-slate-900 dark:text-slate-900 dark:text-white text-[10px] font-bold truncate">
                      {item.template || item.model}
                    </span>
                  </div>
                </div>
              ))
...
export default function GeneratorPage() {
  return (
    <React.Suspense
      fallback={
        <div className="h-full flex items-center justify-center text-slate-900 dark:text-slate-900 dark:text-white">
          Loading...
        </div>
      }
    >
