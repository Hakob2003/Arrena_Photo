...
export function PlansTab() {
  ...
  const confirmDowngrade = async () => {
    ...
    try {
      ...
      console.error(e);
      toast.error(t('billing.plans.updateFailed') || 'Failed to update subscription on server');
    }
  };
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Block 2: Р”РѕСЃС‚СѓРїРЅС‹Рµ С‚Р°СЂРёС„С‹ */}
      <div>
                  ...
                  <ul className="text-sm text-slate-600 dark:text-gray-400 space-y-2 mt-2">
                    {plan.features.map((f, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span className={`${isLuxury ? 'text-[#D4AF37]' : 'text-green-500'} text-xs`}>вњ“</span> {t(f)}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="mt-6">
                  {isCurrent ? (
                    ...
                    <button onClick={() => handleUpgrade(plan.id, plan.priceNum)} className={`w-full py-2 text-sm font-medium rounded-lg transition-colors ${
                      ...
                      {plan.price === '$0' ? (t('billing.plans.downgradeButton') || 'Downgrade') : (t('billing.plans.upgradeButton') || 'Upgrade')}
                    </button>
                  )}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Block 3: РџРѕРїРѕР»РЅРµРЅРёРµ РєСЂРµРґРёС‚РѕРІ */}
        <div>
            ...
            <AnimatePresence>
              {promoApplied && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className={`mt-4 p-3 rounded-lg flex justify-between items-center overflow-hidden border ${isLuxury ? 'bg-[#D4AF37]/10 border-[#D4AF37]/20' : 'bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/20'}`}>
                  ...
                  <span className="text-lg">рџЋ‰</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
