      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {categories.map((cat, i) => (
            <div
              key={i}
              className="bg-white dark:bg-[#0a0a0a] border border-black/10 dark:border-white/10 rounded-2xl p-6 shadow-sm"
            >
              <h3 className="text-md font-semibold text-slate-900 dark:text-white border-b border-black/10 dark:border-white/10 pb-3 mb-4">
                {cat.title}
              </h3>
                    ...
                    <div key={j} className={!model.access ? "opacity-50" : ""}>
                      <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          {!model.access && (
                            <span className="text-[10px] bg-black/10 dark:bg-white/10 px-1.5 py-0.5 rounded text-slate-500">
                              Upgrade required
                            </span>
                          )}
                        </div>
                        {model.access && (
                          <span
                            className={`text-xs font-semibold ${isExhausted ? "text-red-500" : isWarning ? "text-amber-500" : "text-slate-500 dark:text-gray-400"}`}
                          >
