...
const PLAN_DETAILS: Record<
  string,
  { name: string; limit: number; price: string; features: string[] }
> = {
  free: {
    name: "Free",
    limit: 100,
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                ...
                >
                  {isActive
                    ...
                    : t("billing.overview.free")}
                </span>
              </h2>
              {isActive && (
                <p className="text-slate-500 dark:text-gray-400 text-sm mt-1">
                  {t("billing.overview.activeUntil")}{" "}
                  {t("billing.overview.date")}
                </p>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/5">
              ...
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                ...
                <span className="text-sm font-normal text-slate-500">
                  {t("billing.overview.outOf")}{" "}
                  {currentPlan.limit > 100000
                    ? "∞"
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/5">
              <p className="text-sm text-slate-500 dark:text-gray-400 mb-1">
                {t("billing.overview.usedPeriod")}
              </p>
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                {Math.max(0, currentPlan.limit - credits).toLocaleString()}{" "}
                <span className="text-sm font-normal text-slate-500">
                  {t("billing.overview.creditsWord")}
                </span>
              </p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-[#0a0a0a] border border-black/10 dark:border-white/10 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
              {t("billing.overview.manage")}
            </h3>
            <p className="text-sm text-slate-500 dark:text-gray-400 mt-1">
              {t("billing.overview.manageDesc")}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
            ...
            {isActive && (
              <button
                ...
                className="px-4 py-2 bg-transparent border border-black/10 dark:border-white/10 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 dark:hover:text-red-400 hover:border-red-200 dark:hover:border-red-500/30 text-slate-700 dark:text-gray-300 text-sm font-medium rounded-lg transition-all"
              >
        ...
        <div className="bg-white dark:bg-[#0a0a0a] border border-black/10 dark:border-white/10 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-600 dark:text-gray-300">
                  {t("billing.overview.usedCredits")}
                </span>
              ...
              <div className="w-full bg-slate-100 dark:bg-white/10 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${isLuxury ? "bg-[#D4AF37]" : "bg-indigo-500"}`}
                  style={{ width: "75%" }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-600 dark:text-gray-300">
                  {t("billing.overview.imagesGenerated")}
                </span>
                <span className="font-semibold text-slate-900 dark:text-white">
                  340
                </span>
              ...
              <div className="w-full bg-slate-100 dark:bg-white/10 rounded-full h-2">
                <div
                  className={`${isLuxury ? "bg-[#D4AF37]" : "bg-pink-500"} h-2 rounded-full`}
                  style={{ width: "45%" }}
                ></div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            >
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
                {t("billing.overview.cancelModalTitle")}
              </h2>
              <p className="text-slate-600 dark:text-gray-300 text-sm mb-6 leading-relaxed">
                {t("billing.overview.cancelModalDesc1")}{" "}
                <strong>{currentPlan.name}</strong>.{" "}
                {t("billing.overview.cancelModalDesc2")}
              </p>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  onClick={() => setCancelModalOpen(false)}
                  className="w-full px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-sm font-medium rounded-lg transition-colors"
                >
