      ...
      prev.map((g) => {
        if (g.id === id) {
          return {
            ...g,
            isLiked: !g.isLiked,
            likesCount: g.isLiked
              ? Math.max(0, g.likesCount - 1)
              : g.likesCount + 1,
          };
        }
        return g;
      ...
      <div className="w-full">
        <div className="mb-12 text-center space-y-4">
          ...
          <p className="text-base sm:text-lg text-slate-400 dark:text-gray-500 max-w-2xl mx-auto px-2">
            Discover amazing generations from the community. Found something you
            like? Remix it with your own face!
          </p>
        </div>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 overflow-hidden ${isLuxury ? "bg-[#D4AF37]" : "bg-indigo-500"}`}
                      >
                        {gen.user?.image ? (
                          <img
                            src={gen.user.image}
                      ...
                      <span className="text-white text-sm font-semibold truncate w-24">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Heart
                          className={`w-4 h-4 ${gen.isLiked ? "fill-current" : ""}`}
                        />
