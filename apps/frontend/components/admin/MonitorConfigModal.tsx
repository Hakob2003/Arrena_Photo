...
export function MonitorConfigModal({ provider, onClose, onSuccess }: MonitorConfigModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div 
        className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-md flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <h3 className="text-lg font-semibold text-white">Автомониторинг: {provider.name}</h3>
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-white">Включить автопроверку</p>
              <p className="text-xs text-gray-500 mt-1">Регулярно проверять доступность API и баланс</p>
            </div>
