      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold tracking-wide text-white uppercase">
            {t("soc.geo.title") || "Geo Analytics"}
          </h3>
        </div>
        <span className="text-xs font-medium text-white/40">
          {t("soc.geo.topOrigins") || "Top Attack Origins"}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4">
        {data.length === 0 ? (
          <div className="text-center py-8 text-white/40 text-sm">
            {t("soc.geo.noData") || "No geo data available."}
          </div>
        ) : (
          data.map((item, i) => (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.blocked && (
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-400">
                      {t("soc.geo.blocked") || "BLOCKED"}
                    </span>
                  )}
                </div>
                <span className="text-xs font-mono text-white/60">
                  {item.attacks} {t("soc.geo.attacks") || "attacks"}
                </span>
              </div>
