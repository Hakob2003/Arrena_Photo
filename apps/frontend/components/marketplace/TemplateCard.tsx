...
export const TemplateCard: React.FC<{ template: TemplateProps }> = ({
  template,
}) => {
  return (
    <Link
      href={`/marketplace/${template.id}`}
      className="group block rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden hover:shadow-xl transition-all duration-300"
    >
          ...
          {template.oldPrice !== null &&
            ...
            (template.price || 0) > template.oldPrice && (
              <span title={`Previously $${template.oldPrice.toFixed(2)}`}>
                <ArrowUp className="w-3 h-3 text-red-500 ml-1 shrink-0" />
              </span>
            )}
          {template.oldPrice !== null &&
            template.oldPrice !== undefined &&
            (template.price || 0) < template.oldPrice && (
              <span title={`Previously $${template.oldPrice.toFixed(2)}`}>
                <ArrowDown className="w-3 h-3 text-green-500 ml-1 shrink-0" />
              </span>
            )}
        </div>
      </div>
      ...
      <div className="p-4">
        <h3 className="font-semibold text-lg text-gray-900 dark:text-slate-900 dark:text-white truncate">
          {template.name}
        </h3>
        <div className="flex items-center justify-between mt-2">
          <p className="text-sm text-slate-400 dark:text-gray-500 dark:text-gray-400">
