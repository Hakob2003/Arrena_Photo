...
export default function ActivityProfilePage() {
  ...
  useEffect(() => {
    ...
    const savedItemsPerPage = localStorage.getItem("activityItemsPerPage");
      ...
      case "GENERATION":
        return (
          <ImageIcon
            className={cn(
              "w-5 h-5",
              isLuxury ? "text-[#D4AF37]" : "text-blue-400",
            )}
          />
        );
      case "LOGIN":
        return (
          <LogIn
            className={cn(
              "w-5 h-5",
              isLuxury ? "text-[#D4AF37]" : "text-green-400",
            )}
          />
        );
      default:
        return (
          <MonitorPlay className="w-5 h-5 text-slate-500 dark:text-gray-400" />
        );
    }
  };
      ...
      case "SUCCESS":
        return (
          <CheckCircle
            className={cn(
              "w-4 h-4",
              isLuxury ? "text-[#D4AF37]" : "text-green-500",
            )}
          />
        );
      case "ERROR":
      case "FAILED":
        return (
          <XCircle
            className={cn(
              "w-4 h-4",
              isLuxury ? "text-[#D4AF37]" : "text-red-500",
            )}
          />
        );
      default:
        return (
          <Clock
            className={cn(
              "w-4 h-4",
              isLuxury ? "text-[#D4AF37]" : "text-amber-500",
            )}
          />
        );
    }
        ...
        {activity.length === 0 ? (
          ...
          <div className="divide-y divide-white/5">
            {paginatedActivity.map((item, idx) => (
              <div
                key={item.id || idx}
                className="p-4 hover:bg-black/[0.03] dark:bg-white/5 transition-colors flex items-center gap-4"
              >
