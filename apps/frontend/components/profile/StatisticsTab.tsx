...
export default function StatisticsProfilePage() {
  ...
  const statCards = [
    ...
    {
      title: "Member Since",
      value: stats?.registeredAt
        ? format(new Date(stats.registeredAt), "MMM yyyy")
        : "Unknown",
      icon: Calendar,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {statCards.map((card, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-6 flex flex-col gap-4 hover:bg-black/[0.05] dark:bg-white/10 transition-colors"
            >
