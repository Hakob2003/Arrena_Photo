...
function CheckoutForm({ clientSecret, onSuccess }: CheckoutFormProps) {
  ...
  const handleExpressReady = useCallback(
    (event: StripeExpressCheckoutElementReadyEvent) => {
      // availablePaymentMethods will be undefined if nothing is available
  ...
  return (
    <form onSubmit={handleFormSubmit} className="w-full flex flex-col gap-4">
      ...
      {/* Divider between express and card - only if express is available */}
      {expressAvailable && (
      ...
      {/* Card Payment Element - wallets disabled here to avoid duplication */}
      <PaymentElement
        options={{
          wallets: {
            applePay: "never",
