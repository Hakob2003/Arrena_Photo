...
export function Sidebar() {
  ...
  const USER_LINKS = [
    {
      href: "/my-generations",
      label: t("nav.myGenerations"),
      icon: <Clock className="w-5 h-5" />,
    },
    {
      href: "/my-templates",
      label: t("nav.myTemplates"),
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-all relative group ${
                isActive
                  ...
                  : "text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-[#fafafa] dark:hover:bg-white/5"
              ...
              >
                {isNeon && isActive ? (
                  <>
                    ...
                    >
                      <mask
                        id={`icon-mask-${link.href.replace(/[^a-zA-Z0-9]/g, "-")}`}
                      >
                        <svg
                          width="28"
                          height="28"
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundImage:
                            "linear-gradient(135deg, rgb(var(--color-accent-400)), rgb(var(--color-accent-600)), rgb(var(--color-accent-300)))",
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex items-center relative ${isSidebarOpen ? "justify-center p-0" : "justify-start pt-4 pb-2 px-1 border-b border-black/10 dark:border-white/5 md:border-none"}`}
        >
          <div
            className={`flex items-center justify-center relative ${isSidebarOpen ? (isLuxury ? "w-[130px] h-[130px] mx-auto my-4" : "w-[160px] h-[60px] mx-auto my-2") : "w-8 h-8 ml-3"}`}
          >
            <Link
              href="/"
                ...
                {isSidebarOpen && (
                  ...
                  >
                    {isNeon || isPremium ? (
                      <div
                        className={cn(
                          "relative overflow-hidden masked-logo-parent",
                          isLuxury
                            ? "w-[130px] h-[130px]"
                            : "w-[160px] h-[60px]",
                        )}
        ...
        <div
          className={`p-4 border-t border-black/10 dark:border-white/5 mt-auto overflow-hidden ${!isSidebarOpen ? "flex justify-center px-0" : ""}`}
        >
          {user ? (
            <div
              className={`flex ${isSidebarOpen ? "items-center justify-between w-full" : "flex-col items-center justify-center w-full gap-4"}`}
            >
              <Link
                href="/profile"
                className={`flex items-center ${isSidebarOpen ? "gap-3" : "justify-center"} cursor-pointer hover:opacity-80 transition-opacity`}
              >
                <div
                  className={`w-9 h-9 shrink-0 rounded-full flex items-center justify-center font-bold overflow-hidden ${
                    isLuxury
                      ? "bg-gradient-to-tr from-[#C5A028] to-[#D4AF37] text-black"
                ...
                <AnimatePresence mode="wait">
                  {isSidebarOpen && (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-gray-500 truncate uppercase tracking-wider">
                        {user.role}
                      </p>
                    </motion.div>
                  )}
              ...
              <button
                ...
                className={`p-2 text-slate-500 hover:text-slate-900 dark:text-gray-400 dark:hover:text-white transition-colors shrink-0 ${isSidebarOpen ? "ml-2" : ""}`}
                title={t("auth.logout")}
              >
                <svg
                  className="w-5 h-5"
            ...
            <div className="flex flex-col gap-2 w-full">
              <Link
                href="/login"
                className="w-full py-2 text-center text-sm text-slate-500 hover:text-slate-900 hover:bg-[#fafafa] dark:text-gray-300 dark:hover:text-white dark:hover:bg-white/5 rounded-lg transition-colors"
              >
                {t("auth.login")}
              </Link>
              <Link
                href="/register"
                className={`w-full py-2 text-center text-sm font-medium rounded-lg transition-colors ${
                  isLuxury
