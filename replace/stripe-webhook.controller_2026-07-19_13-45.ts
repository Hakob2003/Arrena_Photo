...
export class StripeWebhookController {
  ...
  async handleWebhook(
    ...
    try {
      if (!webhookSecret) {
        ...
        this.logger.warn(
          "No STRIPE_WEBHOOK_SECRET found, trusting event body (TEST MODE ONLY).",
        );
        event = req.body;
      } else {
        if (!req.rawBody) {
          this.logger.error(
            "Raw body is missing! Check NestJS rawBody configuration.",
          );
  ...
  private async handleSubscriptionUpdated(subscription: any) {
    ...
    if (subscription.status === "active") {
      ...
      this.logger.log(
        `Subscription ${subscription.id} active for user ${userId}`,
      );
    }
  }
