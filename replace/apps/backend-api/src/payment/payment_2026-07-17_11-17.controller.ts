import { Controller, Post, Body, UseGuards, InternalServerErrorException, Get } from "@nestjs/common";
import { PaymentService } from "./payment.service";
import { JwtAuthGuard } from "../auth/guards/jwt-auth.guard";
import { CurrentUser } from "../auth/decorators/current-user.decorator";
import { CreatePaymentIntentDto, CreateSubscriptionDto, ProcessWalletDto, ValidateMerchantDto } from "./dto/payment.dto";
...
export class PaymentController {
  ...
  async getConfig() {
    ...
    const publishableKey = this.configService.get("NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY") || this.configService.get("STRIPE_PUBLISHABLE_KEY");
  ...
  async processWallet(
    ...
    try {
      ...
      throw new InternalServerErrorException(e.message || "Failed to process wallet payment");
    }
  }
